      ...
      try {
        ...
        if (formData.role === 'FACULTY') {
          ...
          if (!facultyId && createdUser?.id) {
            ...
            const results = Array.isArray(list?.results)
              ? list.results
              : Array.isArray(list)
                ? list
                : [];
      ...
      try {
        ...
        if ((editingUser?.role || formData.role) === 'FACULTY') {
          const selectedGroupIds = Array.isArray(formData.groups)
            ...
            : [];
          let facultyId =
            updatedUser?.faculty_profile?.id ??
            editingUser?.faculty_profile?.id;
          if (!facultyId && (updatedUser?.id || editingUser?.id)) {
            const list = await facultyApi.list({
              user: updatedUser?.id || editingUser?.id,
            });
            const results = Array.isArray(list?.results)
              ? list.results
              : Array.isArray(list)
        ...
        if (
          (editingUser?.role || formData.role) === 'STUDENT' &&
          formData.group
        ) {
          try {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="rounded-lg bg-white p-3 shadow md:p-6">
        <div className="mb-4 flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 md:mb-6">
          <h2 className="text-xl font-bold text-gray-900 md:text-2xl">
            User Management
          </h2>
          <button
            onClick={() => openModal()}
            className="w-full rounded-md bg-indigo-600 px-4 py-2 text-white transition-colors hover:bg-indigo-700 sm:w-auto"
          >
        ...
        {/* Search and Filter */}
        <div className="mb-4 flex flex-col gap-4 sm:flex-row md:mb-6">
        ...
        {isLoading ? (
          ...
          <>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="flex flex-wrap gap-1">
                          {(() => {
                            ...
                            if (
                              user.role === 'FACULTY' &&
                              user.faculty_profile?.groups?.length > 0
                            ) {
                              return user.faculty_profile.groups.map(
                                (group, index) => (
                                  <span
                                    key={group.id || index}
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-3 flex space-x-2">
                        <button
                          onClick={() => openModal(user)}
                          className="inline-flex flex-1 items-center justify-center rounded-md border border-transparent bg-indigo-100 px-3 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-200"
                        >
                          Edit
                        </button>
                        <button
                          onClick={() => handleDelete(user.id)}
                          className="inline-flex flex-1 items-center justify-center rounded-md border border-transparent bg-red-100 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-200"
                        >
                          Delete
                        </button>
                      </div>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="mt-4 flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0 md:mt-6">
                ...
                <div className="flex justify-center gap-2 sm:justify-end">
                  <button
                    ...
                    className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 md:px-4"
                  >
                    Previous
                  </button>
                    ...
                    .map((page, index, array) => (
                      <React.Fragment key={page}>
                        {index > 0 && array[index - 1] !== page - 1 && (
                          <span className="hidden px-2 py-2 text-sm text-gray-500 sm:inline">
                            ...
                          </span>
                        )}
                        <button
                          onClick={() => setCurrentPage(page)}
                          className={`rounded-md px-3 py-2 text-sm font-medium md:px-4 ${
                            currentPage === page

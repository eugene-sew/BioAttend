...
const FacultyDashboard = () => {
  ...
  const {
    ...
    queryFn: () =>
      // Request minimal page to get an accurate total count without over-fetching
      userApi
        .getFacultyStudents({ page: 1, page_size: 1 })
        .then((r) => r?.data ?? r),
    retry: false,
  });
  ...
  const studentCount = useMemo(() => {
    ...
    if (!d) return 0;
    // Prefer paginated count if available
    if (typeof d?.count === 'number') return d.count;
    // Fall back to results length for non-paginated arrays under data.results
    if (Array.isArray(d?.results)) return d.results.length;
    // Or if the API returns a bare array
  ...
  const classesTodayCount = Array.isArray(todaySchedules)
    ? todaySchedules.length
    : 0;

  // Safely format start/end times. Handles ISO strings and time-only strings (HH:MM or HH:MM:SS).
  const formatTimeLabel = (startRaw, endRaw) => {
    const parseToDate = (raw) => {
      if (!raw) return null;
      // Try native Date parsing first (ISO strings)
    ...
    <div className="space-y-6">
        ...
        {todayLoading ? (
          ...
          <ul className="divide-y divide-gray-200">
            {todaySchedules.map((cls) => {
              ...
              const groupName =
                cls?.group?.name ||
                cls?.assigned_group?.name ||
                cls?.group_name ||
                '—';
              const timeLabel = formatTimeLabel(cls?.start_time, cls?.end_time);
              return (
                <li
                  key={cls.id || `${title}-${timeLabel}`}
      ...
      <div className="rounded-lg bg-white p-6 shadow">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          >
            <svg
              className="mx-auto mb-2 h-8 w-8 text-indigo-600"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >

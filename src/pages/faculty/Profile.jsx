        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {faculty ? (
              <dl className="space-y-2">
                <div>
                  <dt className="text-sm text-gray-500">Faculty ID</dt>
                  <dd className="text-sm font-medium text-gray-900">{faculty.faculty_id}</dd>
                </div>
                {faculty.department && (
                  <div>
                    <dt className="text-sm text-gray-500">Department</dt>
                    <dd className="text-sm font-medium text-gray-900">{faculty.department}</dd>
                  </div>
                )}
                {Array.isArray(faculty.groups) && faculty.groups.length > 0 && (
                  <div>
                    <dt className="text-sm text-gray-500">Assigned Groups</dt>
                    <dd className="text-sm font-medium text-gray-900">
                      {faculty.groups.map((g) => g.code || g.name).filter(Boolean).join(', ')}
                    </dd>
                  </div>
                )}
              </dl>
            ) : (
              <p className="text-sm text-gray-600">No faculty profile found.</p>
            )}
          </div>
        </div>
      </div>
